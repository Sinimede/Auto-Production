...
class DxfFrame(BaseFrame):
    ...
    def _build_ui(self):
        ...
        # Initialize with global out path if available
        ...
        self._check_vars = {
            "laser": tk.BooleanVar(value=True),
            "protecoes": tk.BooleanVar(value=True)
        }
        ...
        tk.Checkbutton(
            checks_frame, text="Laser", variable=self._check_vars["laser"],
        ...
        tk.Checkbutton(
            checks_frame, text="Proteções", variable=self._check_vars["protecoes"],
            bg=BG_CONTENT, fg=TEXT, activebackground=BG_CONTENT,
            activeforeground=TEXT_WHITE, selectcolor=BG_CONTENT,
            font=FONT_LABEL, cursor="hand2",
        ).pack(anchor="w", pady=1)
